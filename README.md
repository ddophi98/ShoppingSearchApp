...
#### DTO -> VO 변환
- 현재 프로젝트에서는 공부를 위해 DTO의 모든 프로퍼티를 옵셔널로 정의하고, VO에서는 옵셔널 처리를 해줬습니다.
- 다만 실제로는 DTO에서 크게 중요하지 않은 값만 옵셔널로 정의를 하고, 중요한 값이라면 값이 들어오지 않았을 때의 에러 처리를 해야한다고 생각합니다.
- 옵셔널 처리 외에도 DTO에서 VO로 변환해줄 때 해야하는 작업이 더 있을 수도 있습니다.   
  ex) 주식 앱이라면 매우 긴 숫자 데이터를 담기 위한 클라이언트 자체 모델로 변환
